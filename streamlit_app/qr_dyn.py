...
# =========================================================
def run_qr_detection(session_dir, fast_mode=True, rate_limit=4):
    """
    Wrapper function to run QR detection as an imported module.
    ...
    """
    global SESSION_DIR, IMAGES_FOLDER, OUTPUT_JSON_RAW, OUTPUT_JSON_CLEAN, DEBUG_DIR
    ...
    OUTPUT_JSON_RAW = SESSION_DIR / "final_superqr_v6_raw.json"
    OUTPUT_JSON_CLEAN = SESSION_DIR / "final_superqr_v6_clean.json"
    DEBUG_DIR = SESSION_DIR / "_debug"
    ...
    try:
        ...
        output_files = []
        if OUTPUT_JSON_RAW.exists():
            output_files.append(OUTPUT_JSON_RAW)
        if OUTPUT_JSON_CLEAN.exists():
            output_files.append(OUTPUT_JSON_CLEAN)
        
        if output_files:
            print(f"✅ QR detection completed: {len(output_files)} file(s) created")
